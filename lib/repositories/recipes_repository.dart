  ...
  try {
    ...
    if (token != null) {
      ...
      var request = http.Request(
          'GET', Uri.parse('$GET_10_RECIPES_FROM_API?userID=$uid'));
...
Future<List<RecipesCard>> get5bestRecipes() async {
  try {
    ...
    if (token != null) {
      ...
      int uid = payload['id'];
      var request = http.Request(
          'GET', Uri.parse('$GET_5_BEST_RECIPES_FROM_API?userID=$uid'));
      http.StreamedResponse response = await request.send();
      final respStr = await response.stream.bytesToString();
      if (response.statusCode == 200) {
        Map<String, dynamic> parse = jsonDecode(respStr);
      ...
      final response =
          await http.get(Uri.parse('$GET_5_BEST_RECIPES_FROM_API'));
      print(response.statusCode.toString() + " get 10 recipes");
      if (response.statusCode == 200) {
        Map<String, dynamic> parse =
            jsonDecode(utf8.decode(response.bodyBytes));
...
Future<List<RecipesCard>> getRecipesbyUserID() async {
  String? token = await LocalData().getToken();
  Map<String?, dynamic> payload = Jwt.parseJwt(token!);
  int uid = payload['id'];
  token = 'Bearer ' + token;
  try {
    final response = await http.get(
        Uri.parse('$GET_USER_RECIPES_FROM_API$uid?page=1&limit=100'),
        headers: {
          HttpHeaders.contentTypeHeader: "application/json",
          HttpHeaders.acceptHeader: "*/*",
          HttpHeaders.authorizationHeader: token
        });
    print('$GET_USER_RECIPES_FROM_API$uid?page=1&limit=100');
    print(token);
  ...
  try {
    ...
    if (token != null) {
      final response = await http
          .get(Uri.parse('$GET_ALL_RECIPES?page=1&limit=100&userID=$uid'));
...
Future<ListRecipes2?> getRecipeVisitor(int page, int limit, int id) async {
  final url =
      "http://14.161.47.36:8080/hiepphat-0.0.1-SNAPSHOT/api/recipes/getallbyuserIDdifferent/$id?page=$page&limit=$limit";
...
Future<bool> privateRecipes(int recipeId) async {
  try {
    String? token = await LocalData().getToken();
    Map<String?, dynamic> payload = Jwt.parseJwt(token!);
    int uid = payload['id'];
    token = 'Bearer ' + token;
    final response =
        await http.put(Uri.parse('$SET_RECIPE_PRIVATE$recipeId'), headers: {
      HttpHeaders.contentTypeHeader: "application/json",
      HttpHeaders.acceptHeader: "*/*",
      HttpHeaders.authorizationHeader: token
    });
    print('$SET_RECIPE_PRIVATE$recipeId');
    print(response.statusCode.toString() + "setprivate thu");
    if (response.statusCode == 200) {
      return true;
    } else {
      return false;
...
Future<ListRecommend?> recommendRecipe() async {
  try {
    String? token = await LocalData().getToken();
    Map<String?, dynamic> payload = Jwt.parseJwt(token!);
    int uid = payload['id'];
    token = 'Bearer ' + token;
    final response =
        await http.get(Uri.parse('$RECOMMENT_RECIPE_FOR_USER$uid'), headers: {
      HttpHeaders.contentTypeHeader: "application/json",
      HttpHeaders.acceptHeader: "*/*",
      HttpHeaders.authorizationHeader: token
    });
    print('$RECOMMENT_RECIPE_FOR_USER$uid');
    print(response.statusCode.toString() + "get recommend");
    if (response.statusCode == 200) {
      Map<String, dynamic> parse = jsonDecode(utf8.decode(response.bodyBytes));
      var list = ListRecommend.fromJson(parse);
      return list;
    } else {
      return null;
    }
  } catch (exception) {
    print('Recommend Recipe error: ' + exception.toString());
    return null;
  }
}
